...
export function parseBudgets(approvedPlan, unknownChannels, inHouseChannels, projectedPlan) {
  ...
  const inHouseMonth = {};
  inHouseChannels && inHouseChannels.forEach(channel => {
    inHouseMonth[channel] = -1;
  });
...
function parseMonth(title, budget, month, current, channel, approvedBudgets) {
  if (title.length === 1) {
    ...
    if (current && current[title[0]]) {
      ...
      obj[title[0]] = {
        values: [],
        children: parseMonth(title.splice(1, title.length - 1), budget, month, current ? (current[title[0]] ? current[title[0]].children : undefined) : undefined, channel, approvedBudgets)
      };
      for (let i = 0; i < month; i++) {
        obj[title[0]].values.push(0);
      }
