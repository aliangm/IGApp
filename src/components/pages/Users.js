      ...
      .map((user, index) => {
        ...
        return this.getTableRow(null, [
          <div className={this.classes.container}>
            <div className={this.classes.icon} style={{ backgroundImage: 'url(https://logo.clearbit.com/' + domain + ')', width: '25px', height: '25px' }}/>
            <div className={this.classes.account}>
              {user.accountName ? user.accountName : domain && domain.match('[^.]+(?=\\.)') && domain.match('[^.]+(?=\\.)')[0]}<div className={this.classes.email}>{emails.length <= 1 ? emails && emails[0] : 'multiple users'}</div>
            </div>
          </div>,
          <div className={this.classes.container}>
          ...
          user.journey.length,
          <div className={this.classes.container}>
            { countries && countries.map(item => <div key={item.code} className={this.classes.container}><ReactCountryFlag code={item.code} svg/><div style={{ marginLeft: '5px' }}>{item.code}</div></div>) }
          </div>,
          this.timeSince(firstTouchPoint),
          this.timeSince(lastTouchPoint),
          <div className={this.classes.container}>
            { devices && devices.map(item => <div key={item} className={this.classes.icon} data-icon={"device:" + item}/>) }
          </div>
        ], {
          key: index,
