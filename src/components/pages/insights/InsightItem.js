...
import {formatTextByType} from 'components/utils/textFormatter';
...
export default class InsightItem extends Component {
  ...
  textForForcastObjectives(leftSideObjectives,objectivesRatio,suggestedBudget,currentBudget){
    if(objectivesRatio.every(objective=>objective.ratio==0)&&(suggestedBudget<=currentBudget)){
        return <div>would make no impact on your forecasted MQLs or other funnel metrics.</div>
    }
    else{
      return <div>
      ...
      return <div className={this.classes.text}>
          {(currentBudget && suggestedBudget) ?
              <div>
                  ...
                  in <b>{dates}</b> by <b>{Math.round((suggestedBudget > currentBudget ? suggestedBudget / currentBudget - 1 : currentBudget / suggestedBudget - 1) * 100)}%</b>, {this.textForForcastObjectives(leftSideObjectives,objectivesRatio, suggestedBudget, currentBudget)}
              </div>
              :
              <div>
                  {suggestedBudget ? 'Adding' : 'Removing'}
                  <b>{channelNickname}</b> {suggestedBudget ? 'to' : 'from'} your mix
                  in <b>{dates}</b>, {this.textForForcastObjectives(leftSideObjectives,objectivesRatio, suggestedBudget, currentBudget)}
              </div>
          }
                  ...
                  .map((item, index) => <div key={index}>
                      - <b>{item.nickname}</b> {(item.ratio == 0 && suggestedBudget <= currentBudget)?  'will have to effect' :
                          <span>{item.ratio >= 0 ? 'by' : 'only by'}
                            <b>{formatTextByType('percentage', item.ratio)}</b> ({(item.projected < 0 ? '-' : '+') + formatBudget(Math.abs(item.projected))})<br/>
                          </span>
                      }
                  </div>);
  }
          ...
          <div className={this.classes.frame}>
              ...
              <div className={this.classes.rightSide} data-green={!(currentBudget && suggestedBudget)}>
                  ...
                  <div className={this.classes.summaryText}>
                      {(objectivesRatio[0] && objectivesRatio[0].ratio) > 0 ? 'Improve' : 'Decline'} in
                      forecasted {objectivesRatio[0] && objectivesRatio[0].nickname}{rightSideObjectives}
                  </div>
              </div>
          </div>
      </div>
