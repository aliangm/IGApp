...
class PathChart extends Component {
	...
	get currentObjective() {
		return this.props.objectives[this.state.currentObjective]
	}

	get chartData() {
		const { months } = this.state
		const { future, past } = this.props.data
	...
	handleObjectiveChange = (shift) => () => {
		const objectivesCount = this.props.objectives.length
	...
	handlePrev = this.handleObjectiveChange(-1)
	handleNext = this.handleObjectiveChange(1)
	...
	render() {
		const { maxMonths, channels } = this.props
